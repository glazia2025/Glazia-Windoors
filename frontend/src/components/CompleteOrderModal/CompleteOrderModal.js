...
const CompleteOrderModal = (props) => {
  const { isOpen, title, message, order, onConfirm, onClose } = props;
  ...
  const [biltyDoc, setBiltyDoc] = useState(null);
  ...
  const doOnConfirm = async () => {
    if (order.isComplete) {
      toast.error("Order already complete");
      return;
    }
    ...
    if (!biltyDoc) {
      handleErrors("bilty_doc", "Bilty Document is required to complete the order");
      hasErrors = true;
    }

    if (!eWayBill) {
      handleErrors("e_way_bill", "EWay Bill is required to complete the order");
      hasErrors = true;
    }
    ...
    if (isConfirmed) {
      let biltyDocBase64 = null;
      let eWayBillBase64 = null;
      let taxInvoiceBase64 = null;

      try {
        if (biltyDoc) {
          biltyDocBase64 = await convertFileToBase64(biltyDoc);
        }
  ...
  const renderPdfOrImage = (proof) => {
    if (!proof || !proof.length)
      return (
        ...
        >
          Failed to render invalid document
        </div>
      );
  ...
  if (!userRole || !order || order.isComplete) return <></>;
  ...
  return (
    <MDBModal open={isOpen} onClose={doOnClose} tabIndex="-1">
      <MDBModalDialog>
        <MDBModalContent>
          <MDBModalHeader>
            <MDBModalTitle className="fw-bold">Order Completion</MDBModalTitle>
            <MDBBtn
              className="btn-close"
              color="none"
              onClick={doOnClose}
            ...
            <div className="mb-3">
              <>
                ...
                <MDBFile
                  ...
                  // value={biltyDoc}
                  onChange={(e) => {
                    setBiltyDoc(e.target.files[0]);
                    handleErrors("bilty_doc", null);
                  }}
                />
