...
const ProfileSelection = forwardRef(({ onProductSelect, selectedProfiles, onRemoveProduct }, ref) => {
  ...
  const fetchTechSheet = async () => {
    try {
      const response = await api.get(
        `https://api.glazia.in/api/admin/get-tech-sheet?main=profile&category=${activeProfile}&subCategory=${activeOption}`
      );
      setSheetData({
  ...
  const onConfirmRow = (product) => {
    ...
    const selectedProduct = {
      ...
      rate: profileData[activeProfile].rate[activeOption],
      amount: (quantities[key]?.quantity || 0) * (profileData[activeProfile]?.rate[activeOption] || 0)
    };
            ...
            <tr key={product.id}>
              ...
              <td>
                <MDBDropdown>
                  <MDBDropdownToggle color="secondary">
                    {powderCoating[`${activeProfile}-${activeOption}-${product.id}`]?.powderCoating || 'No Color'}
                  </MDBDropdownToggle>
                  <MDBDropdownMenu style={{ padding: '10px', maxHeight: '300px', overflowY: 'auto', width: 'max-content' }}>
                    <MDBDropdownItem 
                      className="d-flex align-items-center cursor-pointer" 
                      onClick={() => handlePowderCoating(activeProfile, activeOption, product.id, 'No Color')}
                    >
                      <MDBIcon style={{ fontSize: '20px', marginRight: '10px' }} fas icon="ban" /> 
                      No Color
                    </MDBDropdownItem>
                    {powderColors.map((colorGroup, groupIndex) => (
                      <React.Fragment key={colorGroup.name + groupIndex}>
                        <div style={{ marginTop: '10px', marginBottom: '5px', fontWeight: '600' }}>
                          {colorGroup.name} -
                        </div>
              ...
              <td className="d-flex align-items-center justify-content-start">
                <MDBInput
                  disabled
                  type="number"
                  value={(quantities[`${activeProfile}-${activeOption}-${product.id}`]?.quantity || 0) * 
                         (profileData[activeProfile]?.rate[activeOption] || 0)}
                  size="sm"
                  style={{ minWidth: '80px' }}
                />
                {!selectedProducts.find((sp) => sp.sapCode === product.sapCode) ? (
                  <MDBBtn
                  disabled={!quantities[`${activeProfile}-${activeOption}-${product.id}`]?.quantity}
...
return (
  <div className="profile-selection">
    ...
    {(sheetData.shutterHeight || sheetData.shutterWidth || sheetData.lockingMechanism || 
      sheetData.glassSize || sheetData.alloy || sheetData.interlock) && (
    ...
    {activeOption && (
      <MDBCard className="mt-4">
        <MDBCardBody style={{overflowX: 'scroll', maxWidth: '100%'}}>
          <div 
            className="table-main-header table-controller d-flex justify-content-between align-items-center mb-3 sticky-top bg-white table-responsive"
          ...
          <h6 className="scroll-right">
            Scroll right <MDBIcon fas icon="angle-double-right" style={{color: '#3b71ca'}}/>
          </h6>
